...
- 이 README (셋업/실행 방법)
- `.gitignore`
- ⚠️ `rmf_demos_ws/` 는 **git에 안 올라감** (`.gitignore` 처리됨).
...
## 2. 오피스 데모 실행
```bash
source rmf_demos_ws/install/setup.bash   # ⚠️ /opt/ros 가 아니라 빌드한 워크스페이스를 source!
...
## 메모
- **환경 source 규칙**: 새 터미널마다 `source rmf_demos_ws/install/setup.bash` 필요.
